var pingKey = ["replace_type", "ping_data", "dtype", "pf", "index", "refresh_type", "oid", "channel", "appversion", "chid","pv_type", "order_class", "loid",
    "exp", "cid", "soid", "order_source", "openApp", "seq", "sub_type", "loc", "server_data", "exp_action", "data", "st","oblackwang"]
...
function reportCheck(report) {
    ...
    if ((report.includes('?') && report.includes('&')) == true) {
        ...
        isKeyRepeat = verifyingRepeatedData(reportKey)
        if (isKeyRepeat == true) {
            console.log('key没有重复');
        }
        else {
            console.log('有重复的key：' + isKeyRepeat);
            return 0;
        }
        isMissingKey=reportKeyCheck(reportKey,pingKey);
        if(isMissingKey==true){
            console.log('key没有缺失')
        }
        else{
            console.log('缺失的key:'+isMissingKey);
        }
        isMoreKey=reportKeyCheck(pingKey,reportKey);
        if(isMoreKey==true){
            console.log('没有多余的key')
        }
        else{
            console.log('多余的key:'+isMoreKey);
        }
    }
    else {
        console.log("请求格式不正确");
